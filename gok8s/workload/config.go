...
func (k *Provider) SyncVersionConfig(kcd *kcd1.KCD, version string) error {
	if kcd.Spec.Config == nil {
		return nil
	}
	cm, err := k.cs.CoreV1().ConfigMaps(k.namespace).Get(kcd.Spec.Config.Name, metav1.GetOptions{})
	if err != nil {
		if k8serr.IsNotFound(err) {
			_, err = k.cs.CoreV1().ConfigMaps(k.namespace).Create(
				newVersionConfig(k.namespace, kcd.Spec.Config.Name, kcd.Spec.Config.Key, version))
			if err != nil {
				...
				return errors.Wrapf(err, "failed to create version configmap from %s/%s:%s",
					k.namespace, kcd.Spec.Config.Name, kcd.Spec.Config.Key)
			}
			return nil
		}
